            ...
            function parseEntries(entries, isHistory) {
                ...
                entries.forEach(function(entry) {
                    ...
                    for (var field in keys) {
                        ...
                        //don't process any keys we already processed
                        if (["Title", "URL", "UserName", "Notes"].indexOf(field) < 0) {
                            // Special routine if it should be the entry password
                            if(field == "Password") {
                                if(_this._boolean(keys[field].$.Protected)) {
                                    newEntry.password = _this._unprotect(keys[field]._);
                                } else {
                                    newEntry.password = keys[field]._;
                                }
                            } else {
                                if (keys[field].$ && _this._boolean(keys[field].$.Protected)) {
                                    newEntry.fields[field] = _this._unprotect(keys[field]._);
                                } else {
                                    newEntry.fields[field] = keys[field];
                                }
                            }
                        }
                    }
